...
module.exports.run = async function({ api, event, Users }) {
  ...
  if (event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
    api.changeNickname(`[ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "kazuma" : global.config.BOTNAME}`, threadID, api.getCurrentUserID());
    ...
    try {
      ...
      (typeof threadData.customJoin == "undefined") ? msg = "👋Welcome {name}.\nWelcome to {threadName}.\n{type} is the {soThanhVien}th member of the group 🥳" : msg = threadData.customJoin;
      msg = msg
